...
describe('cli args', () => {
    ...
    it('rejects detailed summary output', () => {
        expect(() => parseArgs(['-o', 'summary', '-d'])).toThrow('The --detailed flag only applies to --output daily.');
    });
    it('formats a smart daily usage header', () => {
        expect(formatUsageHeader('20260319', '20260320')).toBe('Showing usage from 2026-03-19 to 2026-03-20 (2 days)');
    });
    it('formats a singular summary usage header', () => {
        expect(formatUsageHeader('20260320', '20260320')).toBe('Showing usage from 2026-03-20 to 2026-03-20 (1 day)');
    });
});
